...
def loop(N, epsilon, size, lepolys, eps_flag, equation, a, b, forcing):
	...
	for n in tqdm(range(size)):
		...
		if equation == 'BurgersT':
			...
			u, f, alphas, params = generate(x, D, a, b, lepolys, epsilon, equation, sd, forcing)
			# with Pool(5) as p:
			# 	data_ = p.starmap(generate, [(x, D, a, b, lepolys, epsilon, equation, sd, forcing)]) 
			# 	data.append([*data_, epsilon])
			# 	print(data)
			# 	print(*enumerate(data_), sep='\n')
			# 	exit()
...
# with Pool(5) as p:
# 	data = p.map(func, 5*[(x, equation, sd, forcing)]) 
# 	print(data)