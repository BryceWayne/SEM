...
def plotter(xx, sample, T, epoch):
	...
	plt.plot(range(len(uu)), uu, 'r-o', mfc='none', label='$u$')
	plt.plot(range(len(uu)), uhat[0], 'bo', mfc='none', label='$\\hat{u}$')
...
N, D_in, Filters, D_out = 200, 1, 32, 32
FILE = '10000'
# Load the dataset
...
lg_dataset = LGDataset(pickle_file=FILE, subsample=D_out, transform_f= transform_f) #, transform_f= transform_f, transform_a=transform_a
